...
public class ModArmorItem extends ArmorItem {
            ...
            (new ImmutableMap.Builder<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>>())
                    .put(ModArmorMaterials.PINK_GARNET_ARMOR_MATERIAL,
                            List.of(new StatusEffectInstance(StatusEffects.HASTE, 400, 2, false, false),
                                    new StatusEffectInstance(StatusEffects.JUMP_BOOST, 400, 1, false, false))).build(); // remove build() to add
                //    .put(ModArmorMaterials.PINK_GARNET_ARMOR_MATERIAL,
                //         List.of(new StatusEffectInstance(StatusEffects.HASTE, 400, 2, false, false),
    ...
    private void evaluateArmorEffects(PlayerEntity player) { // matches material and effect
        for (Map.Entry<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>> entry : MATERIAL_TO_EFFECT_MAP.entrySet()) {
            ...
            if(hasCorrectArmorOn(mapArmorMaterial, player)) {
                addStatusEffectForMaterial(player, mapArmorMaterial, mapStatusEffects); // adding effect
            }
        }
    }
    ...
    private boolean hasFullSuitOfArmorOn(PlayerEntity player) {
        ...
        return !helmet.isEmpty() && !breastplate.isEmpty() // is full suit
                && !leggings.isEmpty() && !boots.isEmpty();
    }

    private boolean hasCorrectArmorOn(RegistryEntry<ArmorMaterial> material, PlayerEntity player) {
        for (ItemStack armorStack: player.getInventory().armor) {
            if(!(armorStack.getItem() instanceof ArmorItem)) { // not the pumpkin or elytra, or itll cause a crash
                return false;
            }
        }
