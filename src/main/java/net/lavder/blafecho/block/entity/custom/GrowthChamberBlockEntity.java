package net.lavder.blafecho.block.entity.custom;
...
public class GrowthChamberBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos>, ImplementedInventory {
    ...
    protected final PropertyDelegate propertyDelegate; // synchronize client and server
    ...
    public Text getDisplayName() {
        return Text.translatable("block.blafecho.growth_chamber");
    }
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        nbt.putInt("growth_chamber.progress", progress); // also save crafting progress
    ...
    public void tick(World world, BlockPos pos, BlockState state) { // every tick
        if (hasRecipe()) {
            increaseCraftingProgress();
            markDirty(world, pos ,state); // sync
    ...
    private void resetProgress() {
        this.progress = 0;
        this.maxProgress = 72; // just in case
    }
