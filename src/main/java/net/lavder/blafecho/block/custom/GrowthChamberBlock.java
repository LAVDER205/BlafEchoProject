...
public class GrowthChamberBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    @Override
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) { // gets destroyed
        if(state.getBlock() != newState.getBlock()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if(blockEntity instanceof GrowthChamberBlockEntity) {
                ItemScatterer.spawn(world, pos, ((GrowthChamberBlockEntity) blockEntity)); // drop it content
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
        ...
        if (!world.isClient) {
            ...
            if (screenHandlerFactory != null) {
                player.openHandledScreen(screenHandlerFactory); // open GUI
            }
        }
